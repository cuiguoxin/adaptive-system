...
float moving_average_v2(float const previous, std::vector<float> const& losses,
...
int get_real_continuous_level(int const order, int const level) {
  int temp = 0;
...
void aggregate_gradients(
  ...
  PRINT_INFO;
  return_result.clear();
  for (auto iter = vector_name_tensor.begin(); iter != vector_name_tensor.end();
       iter++) {
