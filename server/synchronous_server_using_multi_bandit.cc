 ...
 public:
  ...
  grpc::Status sendLoss(::grpc::ServerContext* context,
    ...
    if (_vector_loss.size() == _number_of_workers) {
      ...
      std::cout << "iteratino :" << _current_iter_number << ", average loss is "
                << average << std::endl;
      auto now = std::chrono::high_resolution_clock::now();
      std::chrono::duration<double> diff_time = (now - _init_time_point);
      _vector_time_history.push_back(diff_time.count());
      _file_loss_stream << std::to_string(diff_time.count()) << ":: iter num ::"
                        << std::to_string(_current_iter_number)
                        << ":: loss is ::" << average << "\n";
  ...
  grpc::Status sendGradient(ServerContext* context,
                            const NamedGradients* request,
    ...
    if (_vector_map_gradient.size() == _number_of_workers) {
      ...
      quantize_gradients(merged_gradient, &_store_named_gradient, _level);
  ...
  grpc::Status sendState(ServerContext* context, const PartialState* request,
    ...
    if (_vector_partial_state.size() == _number_of_workers) {
      if (_bool_is_first_iteration) {
        ...
        // need not to store last action because _grad_quant_level can represent
        // it
        if (_vector_loss_history.size() != 1) {
          PRINT_ERROR_MESSAGE(
              "when in first iteration, the _vector_loss_history's size must "
              "be 1");
          std::terminate();
        }
